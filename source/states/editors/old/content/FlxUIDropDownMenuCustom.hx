...
/**
 * Dropdown menu widget that supports scrolling the item list
 * up/down via mouse wheel, keyboard arrows, and touch swipes.
 */
class FlxUIDropDownMenuCustom extends FlxUIGroup
{
	...
	{
		skipButtonUpdate = b;
		if (header != null && header.button != null)
	...
	public override function update(elapsed:Float):Void
	{
		...
		if (dropPanel.visible)
		{
			...
			{
				// Touch swipe scrolling (works on Android & iOS and any touch device)
				for (swipe in FlxG.swipes)
				{
					var dx = swipe.endPosition.x - swipe.startPosition.x;
					var dy = swipe.endPosition.y - swipe.startPosition.y;
					if (Math.sqrt(dx * dx + dy * dy) >= 25 && Math.abs(dy) > Math.abs(dx))
					{
						if (dy < 0)
						{
							// Swipe up -> reveal next items
							currentScroll++;
							if (currentScroll >= list.length) currentScroll = list.length - 1;
						}
						else
						{
							// Swipe down -> reveal previous items
							--currentScroll;
				...
				// Mouse wheel & keyboard (desktop / physical keyboard)
				// flixel: wheel > 0 = 向上滚, wheel < 0 = 向下滚
				// 向下滚 / 按 ↓ / PageDown -> 显示更靠后的条目 (currentScroll++)
				// 向上滚 / 按 ↑ / PageUp   -> 显示更靠前的条目 (currentScroll--)
				if (FlxG.mouse.wheel < 0 || FlxG.keys.justPressed.DOWN || FlxG.keys.justPressed.PAGEDOWN)
				{
					currentScroll++;
					if (currentScroll >= list.length) currentScroll = list.length - 1;
					updateButtonPositions();
				}
				else if (FlxG.mouse.wheel > 0 || FlxG.keys.justPressed.UP || FlxG.keys.justPressed.PAGEUP)
				{
					--currentScroll;
					if (currentScroll < 0) currentScroll = 0;
					updateButtonPositions();
