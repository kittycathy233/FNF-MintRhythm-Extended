...
class VUIBox extends FlxSpriteGroup
{
	...
	public var tabs(default, null):Array<VUITab> = [];
	
	public var selectedTab(default, set):VUITab = null;
	...
	public function new(x:Float, y:Float, width:Int, height:Int, tabs:Array<String> = null)
	{
		...
		if(tabs != null)
		{
			for (tab in tabs)
			{
				var createdTab:VUITab = new VUITab(tab);
	...
	override function update(elapsed:Float)
	{
		...
		else
		{
			...
			if(wasDragging && broadcastBoxEvents) VUIEventHandler.event(DROP_EVENT, this);
		}
			...
			for (tab in tabs)
			{
				if(FlxG.mouse.overlaps(tab, camera))
				{
					...
					if(!_draggingBox && canMove && _pressedBox && FlxG.mouse.pressed && (Math.abs(FlxG.mouse.deltaScreenX) > 1 || Math.abs(FlxG.mouse.deltaScreenY) > 1))
					{
						...
						_draggingBox = true;
						if(broadcastBoxEvents) VUIEventHandler.event(DRAG_EVENT, this);
					}
					...
					if(FlxG.mouse.justPressed)
					{
						...
						selectedTab = tab;
						_lastClick = 0;
						if(broadcastBoxEvents) VUIEventHandler.event(CLICK_EVENT, this);
					}
		...
		if(_ignoreTabUpdate)
		{
			if(broadcastBoxEvents)
				VUIEventHandler.event(MINIMIZE_EVENT, this);
		}
	...
	{
		var createdTab:VUITab = new VUITab(name);
	...
	private function set_selectedTab(v:VUITab)
	{
		if(v != null)
		{
