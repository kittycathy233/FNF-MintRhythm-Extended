...
import states.editors.vanilla104.content.ui.VUIButton;
import states.editors.vanilla104.content.ui.VUIRadioGroup;
import states.editors.vanilla104.content.ui.VUICheckBox;
import states.editors.vanilla104.content.ui.VUIEventHandler;
import states.editors.vanilla104.content.FileDialogHandler;

class PreloadListSubState extends MusicBeatSubstate implements VUIEvent
{
	...
	var radioGrp:VUIRadioGroup;
	
	var removeButton:VUIButton;
	var lqCheckBox:VUICheckBox;
	var hqCheckBox:VUICheckBox;
	var smCheckBox:VUICheckBox;
	override function create()
	{
		...
		var btn:VUIButton = new VUIButton(bg.x + bg.width - 40, bg.y, 'X', close, 40);
		btn.cameras = cameras;
		add(btn);
		...
		removeButton = new VUIButton(0, 0, 'X', function()
		{
		...
		lqCheckBox = new VUICheckBox(bg.x + bg.width - 100, bg.y + bg.height - 130, 'Low Qual.', 0, updateFilters);
		hqCheckBox = new VUICheckBox(lqCheckBox.x, lqCheckBox.y + 22, 'High Qual.', 0, updateFilters);
		smCheckBox = new VUICheckBox(hqCheckBox.x, hqCheckBox.y + 22, 'Story Mode', 0, updateFilters);
		...
		radioGrp = new VUIRadioGroup(bg.x + 60, bg.y + 80, preloadListKeys, 25, 15, false, 280);
		...
		var loadFileBtn:VUIButton = new VUIButton(0, bg.y + bg.height - 40, 'Load File', function()
		{
		...
		var loadFolderBtn:VUIButton = new VUIButton(0, bg.y + bg.height - 40, 'Load Folder', function()
		{
			if(!fileDialog.completed) return;
		...
		var saveBtn:VUIButton = new VUIButton(0, bg.y + bg.height - 40, 'Save', function()
		{
			if(!fileDialog.completed) return;
	...
	override function update(elapsed:Float)
	{
		...
		var checked:VUIRadioItem = radioGrp.checkedRadio;
		if(checked != null)
	...
	public function UIEvent(id:String, sender:Dynamic)
	{
		...
		switch(id)
		{
			case VUIRadioGroup.CLICK_EVENT:
				updateButtons();
		}
	}
