package states.editors.vanilla104;
...
import states.editors.vanilla104.content.MetaNote;
import states.editors.vanilla104.content.VSlice;
import states.editors.vanilla104.content.Prompt;
import states.editors.vanilla104.content.*;
...
class ChartingState extends MusicBeatState implements VUIEventHandler.VUIEvent
{
	...
	{
		// The vanilla 1.0.4 editor is a 4-key-only tool (grid/notes/hit line are hardcoded to 4 lanes).
		// ChartingRouter normally catches this first, but fall back to the enhanced 1.0 editor just in case.
		if(!states.editors.ChartingRouter.currentChartIsFourKey())
		{
			trace('Vanilla104ChartingState: multi-key chart detected, falling back to the 1.0 editor.');
		...
		infoBox = new VUIBox(infoBoxPosition.x #if mobile - 900 #end, infoBoxPosition.y #if mobile - 250 #end, 220, 220, ['Information']);
		...
		mainBox = new VUIBox(mainBoxPosition.x, mainBoxPosition.y, 300, 280, ['Charting', 'Data', 'Events', 'Note', 'Section', 'Song']);
		...
		upperBox = new VUIBox(40, 40, 330, 300, ['File', 'Edit', 'View']);
		...
		// The (unconditional) touchPad.* references in update() require touchPad to never be null,
		// even on desktop. Always create it; just keep it hidden when not on a touch device.
	...
	override function update(elapsed:Float)
	{
		...
		{
			lastFocus = VUIInputText.focusOn;
			return;
		}
		...
		ClientPrefs.toggleVolumeKeys(VUIInputText.focusOn == null);
		...
		{
			if(VUIInputText.focusOn == null) //If not typing anything
			{
		...
		if(VUIInputText.focusOn == null && lastFocus == null)
		{
	...
	var playbackSlider:VUISlider;

	var mouseSnapCheckBox:VUICheckBox;
	var ignoreProgressCheckBox:VUICheckBox;
	var hitsoundPlayerStepper:VUINumericStepper;
	var hitsoundOpponentStepper:VUINumericStepper;
	var metronomeStepper:VUINumericStepper;

	var instVolumeStepper:VUINumericStepper;
	var instMuteCheckBox:VUICheckBox;
	var playerVolumeStepper:VUINumericStepper;
	var playerMuteCheckBox:VUICheckBox;
	var opponentVolumeStepper:VUINumericStepper;
	var opponentMuteCheckBox:VUICheckBox;
	function addChartingTab()
	{
		...
		objY += 25;
		playbackSlider = new VUISlider(50, objY, function(v:Float) setPitch(playbackRate = v), 1, 0.1, 5.0, 200);
		playbackSlider.label = 'Playback Rate';
		
		objY += 60;
		mouseSnapCheckBox = new VUICheckBox(objX, objY, 'Mouse Scroll Snap', 100, function() chartEditorSave.data.mouseScrollSnap = mouseSnapCheckBox.checked);
		mouseSnapCheckBox.checked = chartEditorSave.data.mouseScrollSnap;

		ignoreProgressCheckBox = new VUICheckBox(objX + 150, objY, 'Ignore Progress Warnings', 100, function() chartEditorSave.data.ignoreProgressWarns = ignoreProgressCheckBox.checked);
		ignoreProgressCheckBox.checked = chartEditorSave.data.ignoreProgressWarns;

		objY += 50;
		hitsoundPlayerStepper = new VUINumericStepper(objX, objY, 0.2, 0, 0, 1, 1);
		hitsoundOpponentStepper = new VUINumericStepper(objX + 100, objY, 0.2, 0, 0, 1, 1);
		metronomeStepper = new VUINumericStepper(objX + 200, objY, 0.2, 0, 0, 1, 1);

		objY += 50;
		instVolumeStepper = new VUINumericStepper(objX, objY, 0.1, 0.6, 0, 1, 1);
		instVolumeStepper.onValueChange = updateAudioVolume;
		playerVolumeStepper = new VUINumericStepper(objX + 100, objY, 0.1, 1, 0, 1, 1);
		playerVolumeStepper.onValueChange = updateAudioVolume;
		opponentVolumeStepper = new VUINumericStepper(objX + 200, objY, 0.1, 1, 0, 1, 1);
		opponentVolumeStepper.onValueChange = updateAudioVolume;

		objY += 25;
		instMuteCheckBox = new VUICheckBox(objX, objY, 'Mute', 60, updateAudioVolume);
		playerMuteCheckBox = new VUICheckBox(objX + 100, objY, 'Mute', 60, updateAudioVolume);
		opponentMuteCheckBox = new VUICheckBox(objX + 200, objY, 'Mute', 60, updateAudioVolume);
	...
	var gameOverCharDropDown:VUIDropDownMenu;
	var gameOverSndInputText:VUIInputText;
	var gameOverLoopInputText:VUIInputText;
	var gameOverRetryInputText:VUIInputText;
	var noRGBCheckBox:VUICheckBox;
	var noteTextureInputText:VUIInputText;
	var noteSplashesInputText:VUIInputText;
	function addDataTab()
	{
		var tab_group = mainBox.getTab('Data').menu;
		var objX = 10;
		var objY = 25;
		gameOverCharDropDown = new VUIDropDownMenu(objX, objY, [''], function(id:Int, character:String)
		{
		...
		objY += 40;
		gameOverSndInputText = new VUIInputText(objX, objY, 120, '', 8);
		...
		objY += 40;
		gameOverLoopInputText = new VUIInputText(objX, objY, 120, '', 8);
		...
		objY += 40;
		gameOverRetryInputText = new VUIInputText(objX, objY, 120, '', 8);
		...
		objY += 35;
		noRGBCheckBox = new VUICheckBox(objX, objY, 'Disable Note RGB', 100, updateNotesRGB);
		
		objY += 40;
		noteTextureInputText = new VUIInputText(objX, objY, 120, '');
		...
		noteSplashesInputText = new VUIInputText(objX + 140, objY, 120, '');
	...
	var eventDropDown:VUIDropDownMenu;
	var value1InputText:VUIInputText;
	var value2InputText:VUIInputText;
	...
	function addEventsTab()
	{
		...
		eventDropDown = new VUIDropDownMenu(objX, objY, [], function(id:Int, character:String)
		{
		...
		var objX2 = 140;
		var removeButton:VUIButton = new VUIButton(objX2, objY, '-', function()
		{
		...
		var addButton:VUIButton = new VUIButton(objX2 + 30, objY, '+', function()
		{
			genericEventButton(function(event:EventMetaNote)
			{
		...
		var leftButton:VUIButton = new VUIButton(objX2 + 80, objY, '<', function()
		{
			genericEventButton(function(event:EventMetaNote) curEventSelected = FlxMath.wrap(curEventSelected - 1, 0, event.events.length - 1));
		}, 20);
		var rightButton:VUIButton = new VUIButton(objX2 + 110, objY, '>', function()
		{
			genericEventButton(function(event:EventMetaNote) curEventSelected = FlxMath.wrap(curEventSelected + 1, 0, event.events.length - 1));
		}, 20);
		...
		objY += 70;
		value1InputText = new VUIInputText(objX, objY, 120, '', 8);
		value1InputText.onChange = function(old:String, cur:String) changeEventsValue(cur, 1);
		value2InputText = new VUIInputText(objX + 150, objY, 120, '', 8);
	...
	var susLengthStepper:VUINumericStepper;
	var strumTimeStepper:VUINumericStepper;
	var noteTypeDropDown:VUIDropDownMenu;
	...
	function addNoteTab()
	{
		...
		susLengthStepper = new VUINumericStepper(objX, objY, Conductor.stepCrochet / 2, 0, 0, Conductor.stepCrochet * 128, 1, 80);
		...
		objY += 40;
		strumTimeStepper = new VUINumericStepper(objX, objY, Conductor.stepCrochet, 0, -5000, Math.POSITIVE_INFINITY, 3, 120);
		...
		objY += 40;
		noteTypeDropDown = new VUIDropDownMenu(objX, objY, [], function(id:Int, changeToType:String)
		{
	...
	var mustHitCheckBox:VUICheckBox;
	var gfSectionCheckBox:VUICheckBox;
	var altAnimSectionCheckBox:VUICheckBox;

	var changeBpmCheckBox:VUICheckBox;
	var changeBpmStepper:VUINumericStepper;
	var beatsPerSecStepper:VUINumericStepper;

	function addSectionTab()
	{
		var affectNotes:VUICheckBox = null;
		var affectEvents:VUICheckBox = null;
		var copyLastSecStepper:VUINumericStepper = null;
		...
		mustHitCheckBox = new VUICheckBox(objX, objY, 'Must Hit Sec.', 70, function()
		{
			var sec = getCurChartSection();
			if(sec != null) sec.mustHitSection = mustHitCheckBox.checked;
			updateHeads(true);
		});
		gfSectionCheckBox = new VUICheckBox(objX + 100, objY, 'GF Section', 70, function()
		{
			var sec = getCurChartSection();
			if(sec != null) sec.gfSection = gfSectionCheckBox.checked;
			updateHeads(true);
		});
		altAnimSectionCheckBox = new VUICheckBox(objX + 200, objY, 'Alt Anim', 70, function()
		{
			var sec = getCurChartSection();
		...
		objY += 40;
		changeBpmCheckBox = new VUICheckBox(objX, objY, 'Change BPM', 80, function()
		{
			var sec = getCurChartSection();
			if(sec != null)
			{
		...
		objY += 25;
		changeBpmStepper = new VUINumericStepper(objX, objY, 1, 0, 1, 400, 3);
		...
		beatsPerSecStepper = new VUINumericStepper(objX + 150, objY, 1, 4, 1, 16, 2);
		...
		objY += 40;
		var copyButton:VUIButton = new VUIButton(objX, objY, 'Copy Section', copyNotesOnSection.bind());
		var pasteButton:VUIButton = new VUIButton(objX + 100, objY, 'Paste Section', function()
		{
			pasteCopiedNotesToSection(affectNotes.checked, affectEvents.checked);
		});
		var clearButton:VUIButton = new VUIButton(objX + 200, objY, 'Clear', function()
		{
			for (note in curRenderedNotes)
			{
				if(note == null) continue;
		...
		objY += 25;
		affectNotes = new VUICheckBox(objX, objY, 'Notes', 60);
		affectNotes.checked = true;
		affectEvents = new VUICheckBox(objX + 100, objY, 'Events', 60);

		objY += 32;
		var copyLastSecButton:VUIButton = new VUIButton(objX, objY, 'Copy Last Section', function()
		{
		...
		copyLastSecStepper = new VUINumericStepper(objX + 110, objY + 2, 1, 1, -999, 999, 0);
		
		objY += 40;
		var swapSectionButton:VUIButton = new VUIButton(objX, objY, 'Swap Section', function()
		{
		...
		var duetSectionButton:VUIButton = new VUIButton(objX + 100, objY, 'Duet Section', function()
		{
			var side:Int = -1;
		...
		var mirrorNotesButton:VUIButton = new VUIButton(objX + 200, objY, 'Mirror Notes', function()
		{
			var maxData:Int = GRID_COLUMNS_PER_PLAYER * GRID_PLAYERS;
			for (note in curRenderedNotes)
			{
	...
	var songNameInputText:VUIInputText;
	var allowVocalsCheckBox:VUICheckBox;

	var bpmStepper:VUINumericStepper;
	var scrollSpeedStepper:VUINumericStepper;
	var audioOffsetStepper:VUINumericStepper;

	var stageDropDown:VUIDropDownMenu;
	var playerDropDown:VUIDropDownMenu;
	var opponentDropDown:VUIDropDownMenu;
	var girlfriendDropDown:VUIDropDownMenu;
	...
	function addSongTab()
	{
		...
		songNameInputText = new VUIInputText(objX, objY, 100, 'None', 8);
		songNameInputText.onChange = function(old:String, cur:String) PlayState.SONG.song = cur;

		allowVocalsCheckBox = new VUICheckBox(objX, objY + 20, 'Allow Vocals', 80, function()
		{
			PlayState.SONG.needsVoices = allowVocalsCheckBox.checked;
			loadMusic();
		});
		var reloadAudioButton:VUIButton = new VUIButton(objX + 120, objY, 'Reload Audio', function() loadMusic(true), 80);

		#if (mac || mobile)
		var reloadJsonButton:VUIButton = new VUIButton(objX + 205, objY, 'Reload JSON', function()
		{
		...
		bpmStepper = new VUINumericStepper(objX, objY, 1, 1, 1, 400, 3);
		...
		scrollSpeedStepper = new VUINumericStepper(objX + 90, objY, 0.1, 1, 0.1, 10, 2);
		scrollSpeedStepper.onValueChange = function() PlayState.SONG.speed = scrollSpeedStepper.value;

		audioOffsetStepper = new VUINumericStepper(objX + 180, objY, 1, 0, -500, 500, 0);
		...
		objY += 40;
		playerDropDown = new VUIDropDownMenu(objX, objY, [''], function(id:Int, character:String)
		{
		...
		stageDropDown = new VUIDropDownMenu(objX + 140, objY, [''], function(id:Int, stage:String)
		{
		...
		opponentDropDown = new VUIDropDownMenu(objX, objY + 40, [''], function(id:Int, character:String)
		{
		...
		girlfriendDropDown = new VUIDropDownMenu(objX, objY + 80, [''], function(id:Int, character:String)
		{
	...
	function addFileTab()
	{
		...
		#if !mobile
		var btn:VUIButton = new VUIButton(btnX, btnY, '  New', function()
		{
			var func:Void->Void = function()
			{
				openNewChart();
		...
		btnY++;
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Open Chart...', function()
		{
		...
		btnY += 20;
		#end
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Open Autosave...', function()
		{
			if(!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			...
			var radioGrp:VUIRadioGroup = new VUIRadioGroup(0, 0, fileList, 25, maxItems, false, 240);
			radioGrp.checked = 0;
			...
			openSubState(new BasePrompt(420, hei, 'Choose an Autosave',
				function(state:BasePrompt) {
					...
					var btn:VUIButton = new VUIButton(state.bg.x + state.bg.width - 40, state.bg.y, 'X', state.close, 40);
					...
					var btn:VUIButton = new VUIButton(0, radioGrp.y + radioGrp.height + 20, 'Load', function()
					{
		...
		{
			btnY += 20;
			var btn:VUIButton = new VUIButton(btnX, btnY, '  Open Events...', function()
			{
				if(!fileDialog.completed) return;
				upperBox.isMinimized = true;
				upperBox.bg.visible = false;
				...
				fileDialog.open(function()
				{
					try
					{
						...
						openSubState(new BasePrompt('Events Found! Choose an action.',
							...
							{
								var btnY = 390;
								var btn:VUIButton = new VUIButton(0, btnY, 'Replace All', function()
								{
									for (event in events)
									{
										if(event != null)
								...
								var btn:VUIButton = new VUIButton(0, btnY, 'Add', function()
								{
									for (event in loadedEvents)
										events.push(createEvent(event));
								...
								var btn:VUIButton = new VUIButton(0, btnY, 'Cancel', state.close);
								btn.screenCenter(X);
								btn.x += 125;
								btn.cameras = state.cameras;
								state.add(btn);
		...
		btnY++;
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Save', function()
		{
			if(!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
		...
		#if !mobile
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Save as...', function()
		{
			if(!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
		...
		{
			btnY += 20;
			var btn:VUIButton = new VUIButton(btnX, btnY, '  Save Events...', function()
			{
				if(!fileDialog.completed) return;
				upperBox.isMinimized = true;
		...
		btnY++;
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Reload Chart', function()
		{
			var func:Void->Void = function()
			{
				if(Song.chartPath == null)
		...
		btnY++;
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Save (V-Slice)...', function()
		{
			if(!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
				...
				try
				{
					...
					openSubState(new BasePrompt('Metadata',
						function(state:BasePrompt)
						{
							...
							var btn:VUIButton = new VUIButton(btnX + 100, btnY, 'Cancel', state.close);
							btn.cameras = state.cameras;
							state.add(btn);
							
							var textX = FlxG.width/2 - 155;
							var textY = 360;
							var artistInput:VUIInputText = new VUIInputText(textX, textY, 120, pack.metadata.artist, 8);
							artistInput.cameras = state.cameras;
							artistInput.onChange = function(old:String, cur:String) pack.metadata.artist = cur;

							var charterInput:VUIInputText = new VUIInputText(textX + 190, textY, 120, pack.metadata.charter, 8);
		...
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Psych to V-Slice...', function()
		{
			if(!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			...
			fileDialog.open('song.json', 'Open a Psych Engine Chart JSON', function()
			{
				...
				openSubState(new BasePrompt('Metadata',
					function(state:BasePrompt)
					{
						...
						var artistInput, charterInput, difficultiesInput:VUIInputText = null;
						...
						var textY = 360;
						artistInput = new VUIInputText(textX, textY, 120, pack.metadata.artist, 8);
						artistInput.cameras = state.cameras;
						artistInput.onChange = function(old:String, cur:String) pack.metadata.artist = cur;
	
						charterInput = new VUIInputText(textX + 150, textY, 120, pack.metadata.charter, 8);
						charterInput.cameras = state.cameras;
						charterInput.onChange = function(old:String, cur:String) pack.metadata.charter = cur;

						var diffs:Array<String> = pack.metadata.playData.difficulties;
						if(diffs == null || diffs.length < 0) pack.metadata.playData.difficulties = diffs = ['easy', 'normal', 'hard'];
						difficultiesInput = new VUIInputText(textX, textY + 42, 160, diffs.join(', '), 8);
		...
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  V-Slice to Psych...', function()
		{
			if(!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
		...
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Update (Legacy)...', function()
		{
			if(!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
		...
		btnY++;
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Preview (${(controls.mobileC) ? 'C' : 'F12'})', openEditorPlayState, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Playtest (${(controls.mobileC) ? 'A' : 'ENTER'})', goToPlayState, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);

		btnY++;
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Exit', function()
		{
	...
	function addEditTab()
	{
		...
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Undo', undo, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);

		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Redo', redo, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);

		btnY++;
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Select All', function()
		{
			var sel = selectedNotes;
			selectedNotes = curRenderedNotes.members.copy();
			addUndoAction(SELECT_NOTE, {old: sel, current: selectedNotes.copy()});
		...
		if(SHOW_EVENT_COLUMN)
		{
			btnY++;
			btnY += 20;
			var btn:VUIButton = new VUIButton(btnX, btnY, '  Lock Events', btnWid);
		...
		btnY++;
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Autosave Settings...', btnWid);
		btn.onClick = function()
		{
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			openSubState(new BasePrompt(400, 160, 'Autosave Settings',
				function(state:BasePrompt)
				{
					...
					var checkbox:VUICheckBox = null;
					var timeStepper:VUINumericStepper = null;

					timeStepper = new VUINumericStepper(state.bg.x + 50, state.bg.y + 90, 1, autoSaveCap, 1, 30, 0);
					...
					checkbox = new VUICheckBox(timeStepper.x + 80, timeStepper.y, 'Enabled', 60, function() {
						autoSaveTime = 0;
					...
					var maxFileStepper:VUINumericStepper = new VUINumericStepper(checkbox.x + 140, checkbox.y, 1, backupLimit, 0, 50, 0);
		...
		btnY++;
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Clear All Notes', function()
		{
			var func:Void->Void = function()
			{
				resetSelectedNotes();
		...
		{
			btnY += 20;
			var btn:VUIButton = new VUIButton(btnX, btnY, '  Clear All Events', function()
			{
				var func:Void->Void = function()
				{
					resetSelectedNotes();
	...
	var showLastGridButton:VUIButton;
	var showNextGridButton:VUIButton;
	var noteTypeLabelsButton:VUIButton;
	var vortexEditorButton:VUIButton;
	function addViewTab()
	{
		...
		showLastGridButton = new VUIButton(btnX, btnY, '', function()
		{
		...
		btnY += 20;
		showNextGridButton = new VUIButton(btnX, btnY, '', function()
		{
		...
		btnY++;
		btnY += 20;
		noteTypeLabelsButton = new VUIButton(btnX, btnY, '', function()
		{
		...
		btnY++;
		btnY += 20;
		vortexEditorButton = new VUIButton(btnX, btnY, vortexEnabled ? '  Vortex Editor ON' : '  Vortex Editor OFF', function()
		{
		...
		btnY++;
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Waveform...', function()
		{
			ClientPrefs.toggleVolumeKeys(false);
			openSubState(new BasePrompt(320, 200, 'Waveform Settings',
				function(state:BasePrompt) {
					...
					var check:VUICheckBox = new VUICheckBox(state.bg.x + 40, state.bg.y + 80, 'Enabled', 60);
					...
					var input:VUIInputText = new VUIInputText(check.x, check.y + 50, 60, waveformC, 10);
					...
					var radioGrp:VUIRadioGroup = new VUIRadioGroup(check.x + 120, check.y, ['Instrumental', 'Main Vocals', 'Opponent Vocals']);
					radioGrp.cameras = state.cameras;
		...
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Go to...', function()
		{
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			openSubState(new BasePrompt(420, 200, 'Go to Time/Section:',
				function(state:BasePrompt)
				{
					...
					var timeStepper:VUINumericStepper = new VUINumericStepper(state.bg.x + 100, state.bg.y + 90, 1, Math.floor(curTime)/1000, 0, FlxG.sound.music.length/1000 - 0.01, 2, 80);
					timeStepper.cameras = state.cameras;
					var sectionStepper:VUINumericStepper = new VUINumericStepper(timeStepper.x + 160, timeStepper.y, 1, currentSec, 0, PlayState.SONG.notes.length - 1, 0);
					...
					var btn:VUIButton = new VUIButton(0, timeTxt.y + 30, 'Go To', function()
					{
						curSec = currentSec;
					...
					var btn:VUIButton = new VUIButton(0, btn.y, 'Cancel', state.close);
					btn.cameras = state.cameras;
					btn.screenCenter(X);
					btn.x += 60;
					state.add(btn);
		...
		btnY++;
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Theme...', function()
		{
			if(!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			...
			openSubState(new BasePrompt(500, 260, 'Chart Editor Theme',
				function(state:BasePrompt)
				{
					...
					var btnY = 320;
					var btn:VUIButton = new VUIButton(0, btnY, 'Light', changeTheme.bind(LIGHT));
					btn.screenCenter(X);
					btn.x -= 180;
					btn.cameras = state.cameras;
					state.add(btn);
			
					var btn:VUIButton = new VUIButton(0, btnY, 'Dark', changeTheme.bind(DARK));
					btn.screenCenter(X);
					btn.x -= 60;
					btn.cameras = state.cameras;
					state.add(btn);
					
					var btn:VUIButton = new VUIButton(0, btnY, 'Default', changeTheme.bind(DEFAULT));
					btn.screenCenter(X);
					btn.cameras = state.cameras;
					btn.x += 60;
					state.add(btn);
			
					var btn:VUIButton = new VUIButton(0, btnY, 'V-Slice', changeTheme.bind(VSLICE));
					btn.screenCenter(X);
					btn.x += 180;
					btn.cameras = state.cameras;
					state.add(btn);
					...
					btnY += 60;
					var btn:VUIButton = new VUIButton(0, btnY, 'Custom', changeTheme.bind(CUSTOM));
					btn.screenCenter(X);
					btn.x -= 180;
					btn.cameras = state.cameras;
					state.add(btn);
					...
					var input:VUIInputText = new VUIInputText(0, btnY, 80, customBgC, 10);
					input.maxLength = 6;
					input.filterMode = ONLY_HEXADECIMAL;
					input.forceCase = UPPER_CASE;
					...
					var input:VUIInputText = new VUIInputText(0, btnY, 80, customGridC[0], 10);
					input.maxLength = 6;
					input.filterMode = ONLY_HEXADECIMAL;
					input.forceCase = UPPER_CASE;
					input.screenCenter(X);
					...
					var input:VUIInputText = new VUIInputText(0, btnY + 30, 80, customGridC[1], 10);
					input.maxLength = 6;
					input.filterMode = ONLY_HEXADECIMAL;
					input.forceCase = UPPER_CASE;
					input.screenCenter(X);
					...
					var input:VUIInputText = new VUIInputText(0, btnY, 80, customGridOtherC[0], 10);
					input.maxLength = 6;
					input.filterMode = ONLY_HEXADECIMAL;
					input.forceCase = UPPER_CASE;
					input.screenCenter(X);
					...
					var input:VUIInputText = new VUIInputText(0, btnY + 30, 80, customGridOtherC[1], 10);
					input.maxLength = 6;
					input.filterMode = ONLY_HEXADECIMAL;
					input.forceCase = UPPER_CASE;
					input.screenCenter(X);
		...
		btnY += 20;
		var btn:VUIButton = new VUIButton(btnX, btnY, '  Reset UI Boxes', function()
		{
			mainBox.setPosition(mainBoxPosition.x, mainBoxPosition.y);
			infoBox.setPosition(infoBoxPosition.x, infoBoxPosition.y);
			UIEvent(VUIBox.DROP_EVENT, btn); //to force a save
		}, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);
	}
	...
	public function UIEvent(id:String, sender:Dynamic)
	{
		...
		switch(id)
		{
			case VUIButton.CLICK_EVENT, VUIDropDownMenu.CLICK_EVENT:
				ignoreClickForThisFrame = true;

			case VUIBox.CLICK_EVENT:
				ignoreClickForThisFrame = true;
				if(sender == upperBox) updateUpperBoxBg();

			case VUIBox.MINIMIZE_EVENT:
				if(sender == upperBox)
				{
